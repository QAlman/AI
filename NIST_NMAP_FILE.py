...
def run_scan_and_analyze():
    ...
    target_input = "https://hrl.rtits.ru/"

    # 1. Получаем текущую дату и время
    current_time = datetime.now()

    # 2. Превращаем её в красивый текст (День.Месяц.Год Часы:Минуты)
    formatted_time_file = current_time.strftime("%d.%m.%Y_%H-%M")

    # Имя файла, куда запишется текстовый отчет
    report_filename = f"{formatted_time_file}_nmap_cwe_report.txt"
    ...
    # Аргументы: -sV (определение версий), -F (быстрое сканирование топ-100 портов для теста)
    # Если нужно сканировать абсолютно все порты, замените '-F' на '-p-'
    # nm.scan(hosts=target, arguments='-sV -F -Pn --version-intensity 9 -sC')
    try:
        # nm.scan(hosts=target, arguments='-sV -p 9500 -Pn -v --version-intensity 7')
        # nm.scan(hosts=target, arguments='-Pn -p 9500, 443 -sV target_ip')
    ...
    with open(report_filename, "w", encoding="utf-8") as f:
        ...
        f.write("=========================================================\n")
        f.write("      ОТЧЕТ ОБ УЯЗВИМОСТЯХ БЕЗОПАСНОСТИ СЕТИ (Nmap + CVE + CWE)\n")
        f.write("=========================================================\n\n")
                        ...
                        if vulnerabilities:
                            ...
                            for cve in vulnerabilities:
                                ...
                                if cwe_id and "CWE-" in str(cwe_id):
                                    ...
                                    f.write(f"        Подробнее о типе ошибки: https://cwe.mitre.org/data/definitions/{cwe_num}.html\n")
                        else:
        ...
        f.write(f"Файл сохранен: {formatted_time}\n")
